    ...
    async def listen_for_updates():
        ...
        try:
            async with websockets.connect(uri) as websocket:
                print(f"[OK] Connected to WebSocket: {uri}")

                # Wait for messages for a limited time
                try:
                    # Set a timeout for receiving messages
                    message = await asyncio.wait_for(websocket.recv(), timeout=10.0)
                    print(f"[OK] Received message: {message}")
                    ...
                    try:
                        ...
                        print(f"[FAIL] Invalid JSON in message: {message}")
                        return False

                except asyncio.TimeoutError:
                    print("[INFO] No messages received within timeout period")
...
def test_task_creation_via_chat():
    ...
    if response.status_code == 200:
        ...
        if chat_response.get("tool_calls_executed", False):
            print("[OK] Tool calls executed (task likely created)")
            ...
            if messages_received:
                print(f"[OK] Received {len(messages_received)} WebSocket message(s)")
                for msg in messages_received:
                    print(f"  - {msg}")

                    # Check if it's a task update
                    if msg.get("event") == "task_update":
                        print("  [OK] Confirmed: Task update received via WebSocket")
                        return True

            print("[WARN] No task update messages received via WebSocket")
            return False
        else:
            print("[WARN] No tool calls executed - task may not have been created")
            return False
    else:
        print(f"[FAIL] Chat API call failed: {response.status_code} - {response.text}")
        return False
    ...
    if response.status_code == 200:
        ...
        for task in tasks:
            if "groceries" in task.get("title", "").lower():
                print(f"[OK] Found test task: '{task['title']}' (ID: {task['id']})")
                test_task_found = True
                break

        if test_task_found:
            print("[OK] Task creation via chat confirmed")
            return True
        else:
            print("[WARN] Test task not found in task list")
            return False
    else:
        print(f"[FAIL] Failed to get tasks: {response.status_code} - {response.text}")
        return False
...
def main():
    ...
    print(f"Overall Result: {'[PASS] ALL TESTS PASSED' if overall_success else '[FAIL] SOME TESTS FAILED'}")
    print("="*60)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if health_response.status_code == 200:
            ...
            print(f"[FAIL] Backend not accessible at {BASE_URL}")
