#!/usr/bin/env python3
"""
Practical OAuth Error Testing Script
    ...
    for provider in valid_providers:
        ...
        try:
            ...
            if response.status_code in [302, 307]:
                print("  ✓ Valid provider - redirect to OAuth provider")
            elif response.status_code in [400, 404, 422]:
                print("  ✓ Valid provider - but OAuth not configured (expected in test)")
            elif response.status_code == 401:
                print("  ✓ Valid provider - but OAuth credentials missing (expected in test)")
            else:
        ...
        try:
            ...
            if response.status_code in [302, 307]:
                ...
                if 'error' in location.lower():
                    print("  ✓ Properly redirects with error")
                else:
                    print("  ⚠ Redirects but no error in URL")
            elif response.status_code >= 400:
                print("  ✓ Returns appropriate error status")
            else:
                print(f"  ⚠ Unexpected status: {response.status_code}")
                all_passed = False
