...
class User(SQLModel, table=True):
    ...
    reset_password_expires_at: Optional[datetime] = Field(default=None)
...
class Conversation(SQLModel, table=True):
    ...
    Attributes:
        id: Unique auto-incrementing conversation ID
        user_id: ID of the user who owns this conversation (from Better Auth)
        title: Auto-generated title from first message (optional, max 200 characters)
    ...
    title: Optional[str] = Field(
        ...
        max_length=200,
        nullable=True,
        description="Auto-generated title from first message (optional, max 200 characters)"
    )
    created_at: datetime = Field(
        default_factory=datetime.utcnow,
...
class Message(SQLModel, table=True):
    ...
    Attributes:
        id: Unique auto-incrementing message ID
        conversation_id: ID of the conversation this message belongs to
        role: Role of the sender ('user', 'assistant', or 'system')
        content: Content of the message (max 10000 characters)
    ...
    role: str = Field(
        nullable=False,
        max_length=20,
        description="Role of the sender ('user', 'assistant', or 'system')"
    )
    content: str = Field(
        nullable=False,
        max_length=10000,
        description="Content of the message (max 10000 characters)"
    )
    created_at: datetime = Field(
        default_factory=datetime.utcnow,
...
# Define composite index for user_id and created_at for efficient conversation listing
Index("idx_conversations_user_created", Conversation.user_id, Conversation.created_at)

# Define index for conversation_id and created_at for efficient message retrieval
